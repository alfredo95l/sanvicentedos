...
namespace sanvicentedos.WEB.Repositories
{
    public class Repository : IRepository
    {
        ...
        public async Task<HttpResponseWrapper<object>> Put<T>(string url, T modelo)
        {
        ...
        public async Task<HttpResponseWrapper<TResponse>> Put<T, TResponse>(string url, T modelo)
        {
            var messageJSON = JsonSerializer.Serialize(modelo);
            var messageContent = new StringContent(messageJSON, Encoding.UTF8, "application/json");
            var responseHttp = await _httpClient.PutAsync(url, messageContent);
